...
Covers build_draft_comment_payload(), create_draft_comment(),
create_draft_comments(), list_draft_comments(), delete_draft_comment(),
and publish_draft_comments() with 100% branch coverage.
"""
...
# ===========================================================================
# TestListDraftComments
# ===========================================================================


class TestListDraftComments:
  """Covers every branch in list_draft_comments()."""

  SAMPLE_DRAFTS = {
    "src/foo.cpp": [SAMPLE_DRAFT_INFO],
  }

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_returns_file_keyed_map(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, self.SAMPLE_DRAFTS]
    result = list_draft_comments(mock_ctx, "12345")
    assert result["drafts"] == self.SAMPLE_DRAFTS

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_empty_returns_empty_dict(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    result = list_draft_comments(mock_ctx, "12345")
    assert result["drafts"] == {}

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_uses_current_revision_by_default(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    list_draft_comments(mock_ctx, "12345")
    drafts_endpoint = mock_req.call_args_list[1][0][1]
    assert "abc123def456" in drafts_endpoint

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_with_patchset_number(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    list_draft_comments(mock_ctx, "12345", patchset_number="2")
    drafts_endpoint = mock_req.call_args_list[1][0][1]
    assert "def456abc123" in drafts_endpoint

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_invalid_patchset_raises(self, mock_req, mock_ctx):
    mock_req.return_value = SAMPLE_CHANGE_INFO
    with pytest.raises(ValueError, match="Patchset 77 not found"):
      list_draft_comments(mock_ctx, "12345", patchset_number="77")

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_method_is_get(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    list_draft_comments(mock_ctx, "12345")
    # Default method is GET (no method kwarg needed, or method="GET")
    drafts_call = mock_req.call_args_list[1]
    assert drafts_call[1].get("method", "GET") == "GET"

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_api_failure_propagates(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, Exception("network error")]
    with pytest.raises(Exception, match="network error"):
      list_draft_comments(mock_ctx, "12345")

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_response_has_change_id(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    result = list_draft_comments(mock_ctx, "12345")
    assert result["change_id"] == "12345"

  @patch("server.make_gerrit_rest_request")
  def test_list_drafts_response_has_revision(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    result = list_draft_comments(mock_ctx, "12345")
    assert result["revision"] == "abc123def456"
...
# ===========================================================================
# TestDeleteDraftComment
# ===========================================================================


class TestDeleteDraftComment:
  """Covers every branch in delete_draft_comment()."""

  @patch("server.make_gerrit_rest_request")
  def test_delete_success_returns_confirmation(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    result = delete_draft_comment(mock_ctx, "12345", "draft_id_001")
    assert result["deleted"] is True

  @patch("server.make_gerrit_rest_request")
  def test_delete_endpoint_contains_draft_id(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    delete_draft_comment(mock_ctx, "12345", "my_draft_id")
    delete_endpoint = mock_req.call_args_list[1][0][1]
    assert "my_draft_id" in delete_endpoint
    assert "drafts" in delete_endpoint

  @patch("server.make_gerrit_rest_request")
  def test_delete_method_is_delete(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    delete_draft_comment(mock_ctx, "12345", "draft_id_001")
    delete_call = mock_req.call_args_list[1]
    assert delete_call[1]["method"] == "DELETE"

  @patch("server.make_gerrit_rest_request")
  def test_delete_uses_current_revision_by_default(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    delete_draft_comment(mock_ctx, "12345", "draft_id_001")
    delete_endpoint = mock_req.call_args_list[1][0][1]
    assert "abc123def456" in delete_endpoint

  @patch("server.make_gerrit_rest_request")
  def test_delete_with_patchset_number(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    delete_draft_comment(mock_ctx, "12345", "draft_id_001", patchset_number="2")
    delete_endpoint = mock_req.call_args_list[1][0][1]
    assert "def456abc123" in delete_endpoint

  @patch("server.make_gerrit_rest_request")
  def test_delete_invalid_patchset_raises(self, mock_req, mock_ctx):
    mock_req.return_value = SAMPLE_CHANGE_INFO
    with pytest.raises(ValueError, match="Patchset 55 not found"):
      delete_draft_comment(mock_ctx, "12345", "draft_id_001", patchset_number="55")

  @patch("server.make_gerrit_rest_request")
  def test_delete_api_failure_propagates(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, Exception("404 Not Found")]
    with pytest.raises(Exception, match="404 Not Found"):
      delete_draft_comment(mock_ctx, "12345", "nonexistent_draft")

  @patch("server.make_gerrit_rest_request")
  def test_delete_response_has_draft_id(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    result = delete_draft_comment(mock_ctx, "12345", "draft_id_001")
    assert result["draft_id"] == "draft_id_001"

  @patch("server.make_gerrit_rest_request")
  def test_delete_response_has_revision(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    result = delete_draft_comment(mock_ctx, "12345", "draft_id_001")
    assert result["revision"] == "abc123def456"

  @patch("server.make_gerrit_rest_request")
  def test_delete_response_has_change_id(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, {}]
    result = delete_draft_comment(mock_ctx, "12345", "draft_id_001")
    assert result["change_id"] == "12345"
...
# ===========================================================================
# TestPublishDraftComments
# ===========================================================================


SAMPLE_REVIEW_RESULT = {"labels": {}, "ready": True}


class TestPublishDraftComments:
  """Covers every branch in publish_draft_comments()."""

  @patch("server.make_gerrit_rest_request")
  def test_publish_success(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    result = publish_draft_comments(mock_ctx, "12345")
    assert result["published"] == SAMPLE_REVIEW_RESULT

  @patch("server.make_gerrit_rest_request")
  def test_publish_payload_has_publish_all_revisions(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345")
    payload = mock_req.call_args_list[1][1]["json_payload"]
    assert payload["drafts"] == "PUBLISH_ALL_REVISIONS"

  @patch("server.make_gerrit_rest_request")
  def test_publish_default_notify_is_owner(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345")
    payload = mock_req.call_args_list[1][1]["json_payload"]
  ...
  @patch("server.make_gerrit_rest_request")
  def test_publish_custom_notify_none(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345", notify="NONE")
    payload = mock_req.call_args_list[1][1]["json_payload"]
    assert payload["notify"] == "NONE"

  @patch("server.make_gerrit_rest_request")
  def test_publish_custom_notify_owner_reviewers(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345", notify="OWNER_REVIEWERS")
    payload = mock_req.call_args_list[1][1]["json_payload"]
    assert payload["notify"] == "OWNER_REVIEWERS"

  @patch("server.make_gerrit_rest_request")
  def test_publish_custom_notify_all(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345", notify="ALL")
    payload = mock_req.call_args_list[1][1]["json_payload"]
    assert payload["notify"] == "ALL"

  @patch("server.make_gerrit_rest_request")
  def test_publish_with_message_includes_message(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345", message="LGTM after fixes")
    payload = mock_req.call_args_list[1][1]["json_payload"]
    assert payload["message"] == "LGTM after fixes"

  @patch("server.make_gerrit_rest_request")
  def test_publish_without_message_excludes_key(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345")
    payload = mock_req.call_args_list[1][1]["json_payload"]
  ...
  @patch("server.make_gerrit_rest_request")
  def test_publish_uses_review_endpoint(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345")
    review_endpoint = mock_req.call_args_list[1][0][1]
    assert review_endpoint.endswith("/review")

  @patch("server.make_gerrit_rest_request")
  def test_publish_method_is_post(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345")
    review_call = mock_req.call_args_list[1]
    assert review_call[1]["method"] == "POST"

  @patch("server.make_gerrit_rest_request")
  def test_publish_patchset_resolution(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    publish_draft_comments(mock_ctx, "12345", patchset_number="2")
    review_endpoint = mock_req.call_args_list[1][0][1]
    assert "def456abc123" in review_endpoint

  @patch("server.make_gerrit_rest_request")
  def test_publish_invalid_patchset_raises(self, mock_req, mock_ctx):
    mock_req.return_value = SAMPLE_CHANGE_INFO
    with pytest.raises(ValueError, match="Patchset 88 not found"):
      publish_draft_comments(mock_ctx, "12345", patchset_number="88")

  @patch("server.make_gerrit_rest_request")
  def test_publish_api_failure_propagates(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, Exception("server error")]
    with pytest.raises(Exception, match="server error"):
      publish_draft_comments(mock_ctx, "12345")

  @patch("server.make_gerrit_rest_request")
  def test_publish_response_has_change_id(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    result = publish_draft_comments(mock_ctx, "12345")
    assert result["change_id"] == "12345"

  @patch("server.make_gerrit_rest_request")
  def test_publish_response_has_revision(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    result = publish_draft_comments(mock_ctx, "12345")
    assert result["revision"] == "abc123def456"

  @patch("server.make_gerrit_rest_request")
  def test_publish_response_has_published(self, mock_req, mock_ctx):
    mock_req.side_effect = [SAMPLE_CHANGE_INFO, SAMPLE_REVIEW_RESULT]
    result = publish_draft_comments(mock_ctx, "12345")
