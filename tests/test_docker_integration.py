    ...
    try:
        ...
        container.exec(
            [
                "sh",
                "-c",
                "timeout 5s gerrit-review-mcp 2>&1 | tee /tmp/mcp_startup.log || true",
            ]
        )
...
def gerrit_mcp_container(
    docker_image: str, test_env_file: str
...
class TestDockerIntegrationTestcontainers:
    ...
    def test_container_health_comprehensive(
        self, gerrit_mcp_container: DockerContainer
    ):
        ...
        import_test = (
            "import mcp, requests, dotenv; print('All packages imported successfully')"
        )
    ...
    def test_container_python_environment(self, gerrit_mcp_container: DockerContainer):
        ...
        try:
            result = gerrit_mcp_container.exec(
    ...
    def test_container_file_structure(self, gerrit_mcp_container: DockerContainer):
        ...
        # Required files that must be present at /app root
        expected_files = ["src", "pyproject.toml"]
    ...
    def test_container_interactive_mode_simulation(
        self, docker_image: str, test_env_file: str
    ):
        ...
        try:
            ...
            for i, container in enumerate(containers):
                _wait_for_container_ready(
                    container, TEST_CONFIG["CONTAINER_READY_TIMEOUT"]
                )
    ...
    def test_container_error_recovery(self, docker_image: str, test_env_file: str):
        ...
        bad_container = (
            ...
            .with_command(
                "sh -c 'sleep 1 && exit 1'"
            )  # Command that fails after brief delay
            .with_kwargs(auto_remove=False)
        )
        ...
        try:
            _wait_for_container_ready(
                good_container, TEST_CONFIG["CONTAINER_READY_TIMEOUT"]
            )
    ...
    def test_image_reuse_efficiency(self, docker_image: str, test_env_file: str):
        ...
        try:
            ...
            _wait_for_container_ready(
                container1, TEST_CONFIG["CONTAINER_READY_TIMEOUT"]
            )
            _wait_for_container_ready(
                container2, TEST_CONFIG["CONTAINER_READY_TIMEOUT"]
            )
