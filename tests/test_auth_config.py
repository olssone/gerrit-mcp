...
class TestAuthMethodConfiguration:
    ...
    @pytest.mark.parametrize(
        "invalid_method", ["invalid", "oauth", "token", "", "digest,basic", "none"]
    )
    def test_invalid_auth_methods(self, invalid_method):
        ...
        with patch.dict(os.environ, {"GERRIT_AUTH_METHOD": invalid_method}):
            with pytest.raises(ValueError) as exc_info:
                get_auth_method()
            assert f"Invalid authentication method: '{invalid_method}'" in str(
                exc_info.value
            )
...
class TestPasswordConfiguration:
    ...
    def test_basic_password_configuration(self):
        ...
        with patch.dict(
            os.environ,
            {"GERRIT_AUTH_METHOD": "basic", "GERRIT_PASSWORD": "ldap-password-456"},
            clear=True,
        ):
    ...
    def test_missing_digest_password(self):
        ...
        with patch.dict(os.environ, {"GERRIT_AUTH_METHOD": "digest"}, clear=True):
            with pytest.raises(ValueError) as exc_info:
                get_password()
            assert "GERRIT_HTTP_PASSWORD environment variable is required" in str(
                exc_info.value
            )
    ...
    def test_missing_basic_password(self):
        ...
        with patch.dict(os.environ, {"GERRIT_AUTH_METHOD": "basic"}, clear=True):
            with pytest.raises(ValueError) as exc_info:
                get_password()
            assert "GERRIT_PASSWORD environment variable is required" in str(
                exc_info.value
            )
...
class TestIntegrationScenarios:
    ...
    def test_backward_compatibility_default(self):
        ...
        with patch.dict(
            os.environ, {"GERRIT_HTTP_PASSWORD": "original-password"}, clear=True
        ):
